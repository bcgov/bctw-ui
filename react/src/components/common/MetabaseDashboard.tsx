    ...
    <Box className={'sidebar'} id="manage_sidebar" py={2} px={2}>
      <List component='nav'>
        {dashboardItems.map(item => 
          <ListItem className="side-bar-item" button disabled={item.id === null} onClick={()=>setDashboardKey(item.id)} id={item.name}>
          <ListItemIcon>
            <Icon icon={item.icon} />
          </ListItemIcon>
      ...
      <Box py={3} px={4} className={classes.manageLayoutContent}>
        {/* <iframe src={url} id='iframe' frameBorder={0} style={{minWidth: '100%', minHeight:'100%'}}></iframe> */}
        <IframeResizer
        src={url} 
