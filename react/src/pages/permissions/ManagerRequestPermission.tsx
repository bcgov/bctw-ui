...
export default function ManagerRequestPermission(): JSX.Element {
  ...
  const CritterList = (): JSX.Element => {
    ...
    return p?.length ? (
      <ul style={listStyle}>
        {p.map((e) => (
          <li>{e.animal_id ? `Critter ID: ${e.animal_id}` : e.wlh_id ? `WLH_ID: ${e.wlh_id}` : `no animal`}</li>
        ))}
      </ul>
    ) : null;
  };
    ...
    <ManagerLayout>
      <div className='container'>
        ...
        <Typography variant='body1' component='p'>
          Submit a new animal permission request for another user
        </Typography>
        <ol>
          {AnimalDelegationSteps.map((step: string, idx: number) => (
            <li key={idx}>{step}</li>
          ))}
        </ol>
        <EditTable
          ...
          headers={['', '', 'Emails', 'Critter Identifier', 'Permission', 'Comment', 'Edit', 'Reset']}
        ...
        <PickCritterPermissionModal
          ...
          headersToShow={UserCritterAccess.propsToDisplay.filter(
            (p: keyof UserCritterAccess) => p !== 'permission_type'
          )}
        />
        {error ? (
          <div style={{ marginBottom: '10px' }}>
            <NotificationMessage severity={'error'} message={error} />
          </div>
        ) : null}
        <DataTable
