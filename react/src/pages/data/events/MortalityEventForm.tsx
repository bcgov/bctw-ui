...
import { FormChangeEvent, FormFieldObject, parseFormChangeResult } from 'types/form_types';
...
import { deviceFormFields } from 'types/collar';
import { animalFormFields } from 'types/animal';
...
export default function MortalityEventForm({ event, handleFormChange, handleExitEarly }: MortEventProps): JSX.Element {
  ...
  /**
   * fixme: merge map and type this properly :\
   * doesnt really work well as 
   * 1) you could try to get fields from amap/cmap that only belong to a mortality event (ex. shouldunattachdevice)
   * 2) mortality specific fields require spread operator
   * 
   * todo: get @function createFormField to a FormFieldObject of a type that overlaps but doesnt
   * necessarily contain every prop of T
   */
  const amap = animalFormFields as Map<keyof MortalityEvent, FormFieldObject<MortalityEvent>>;
  const cmap = deviceFormFields as Map<keyof MortalityEvent, FormFieldObject<MortalityEvent>>;
  ...
  return (
    <>
      {FormSection('mort-a-st', event.formatPropAsHeader('animal_status'), [
        <Radio
          propName={amap.get('animal_status').prop}
      ...
      {FormSection('mort-device', 'Event Details', [
        ...
        <LocationEventForm
          ...
          childNextToDate={CreateFormField(mortality, cmap.get('device_status'), onChange, critterIsAlive)}
          children={
            <>
              {/* device retrieval */}
              <Box mb={1} {...boxSpreadRowProps}>
                {CreateFormField(mortality, cmap.get('retrieved'), onChange, critterIsAlive)}
                {CreateFormField(mortality, { ...cmap.get('retrieval_date'), required: isRetrieved }, onChange, !isRetrieved || critterIsAlive)}
              </Box>
        ...
        />,
        <Box mt={2}>
          {CreateFormField(mortality, cmap.get('device_condition'), onChange, critterIsAlive)}
          {CreateFormField(mortality, cmap.get('device_deployment_status'), onChange, critterIsAlive)}
        </Box>,
        CreateFormField(mortality, cmap.get('activation_status'), onChange, critterIsAlive, true)
      ], null, critterIsAlive)}
      ...
      {FormSection('mort-critter', 'Animal Details', [
        <Box {...boxSpreadRowProps}>
          ...
          {CreateFormField(mortality, amap.get('mortality_investigation'), onChange, !wasInvestigated || critterIsAlive)}
        </Box>,
        CreateFormField(mortality, amap.get('mortality_report'), onChange, critterIsAlive, true),
        <Box mt={1}>
          {CreateFormField(mortality, amap.get('proximate_cause_of_death'), onChange, critterIsAlive)}
          {CreateFormField(mortality, amap.get('ultimate_cause_of_death'), onChange, ucodDisabled || critterIsAlive)}
        </Box>,
        <Box mt={1} {...boxSpreadRowProps}>
          {CreateFormField(mortality, amap.get('predator_known'), onChange, !isPredation || critterIsAlive)}
          {CreateFormField(mortality, amap.get('predator_species_pcod'), onChange, !isPredatorKnown || critterIsAlive)}
          {CreateFormField(mortality, amap.get('pcod_confidence'), onChange, !isPredatorKnown || critterIsAlive)}
        </Box>,
        <Box mt={1} {...boxSpreadRowProps}>
          {CreateFormField(mortality, {...fields.isUCODSpeciesKnown}, onChange, !isPredatorKnown || critterIsAlive)}
          {CreateFormField(mortality, amap.get('predator_species_ucod'), onChange, !(isPredatorKnown && isUCODKnown) || critterIsAlive)}
          {CreateFormField(mortality, amap.get('ucod_confidence'), onChange, !(isPredatorKnown && isUCODKnown) || critterIsAlive)}
        </Box>,
